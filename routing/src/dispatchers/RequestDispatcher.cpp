...
{
    auto gen_uuid = boost::uuids::random_generator();
    boost::uuids::uuid request_id;
    do {
        request_id = gen_uuid();
    } while (requests.find(request_id) != requests.end());
...
void RequestDispatcher::onProcessed(boost::uuids::uuid request_id, Message* result)
{
    ...
    requests.erase(it);
    on_processed(std::unique_ptr<Message>(result));
}
